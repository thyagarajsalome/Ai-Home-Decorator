...
const ResultDisplay: React.FC<ResultDisplayProps> = ({ originalImage, generatedImage }) => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto mt-8">
        <h2 className="text-3xl font-bold text-center mb-4 bg-gradient-to-r from-purple-400 to-pink-600 bg-clip-text text-transparent">Your Redesigned Room!</h2>
        <div className="relative w-full aspect-video rounded-lg overflow-hidden shadow-2xl select-none cursor-ew-resize">
            <img 
                src={originalImage} 
                alt="Original Room" 
            ...
            >
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10 bg-white/80 rounded-full shadow-md grid place-items-center backdrop-blur-sm text-gray-800">
            ...
            {/* Labels */}
            <div className="absolute top-2 left-2 py-1 px-2 bg-black/50 rounded-md text-sm font-semibold pointer-events-none">Before</div>
            <div className="absolute top-2 right-2 py-1 px-2 bg-black/50 rounded-md text-sm font-semibold pointer-events-none">After</div>
        </div>
